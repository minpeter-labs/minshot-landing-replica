...
const captureTarget = async (browser, target) => {
  const entries = await Promise.all(
    surfaces.map(async (surface) => {
      ...
      return [
        surface.id,
        await capturePage({ browser, path, surface, target }),
      ];
    })
  );
...
const passBySurface = (comparison) =>
  Object.fromEntries(
    Object.entries(comparison).map(([surface, result]) => [
      surface,
      result.pass,
    ])
  );
...
const footerMatchesLive = (captures, liveCaptures) =>
  Object.fromEntries(
    surfaces.map((surface) => [
      surface.id,
      footerSignature(captures[surface.id]) ===
        footerSignature(liveCaptures[surface.id]),
    ])
  );

const routeIsReachable = (receipt) =>
  [200, 301, 302, 307, 308].includes(receipt.status);
...
const targetPass = (captures, routes) => ({
  ...
  noHorizontalOverflow: Object.fromEntries(
    surfaces.map((surface) => [
      surface.id,
      !captures[surface.id].layout.overflow.horizontal,
    ])
  ),
  routesReachable:
  ...
  zhMobileIsChinese: captures.zhMobile.layout.bodyTextSample.includes(
    "原生 macOS 截图工具"
  ),
});
...
const buildPass = ({ captures, comparisons, routes, targets }) => {
  const strictPass = Object.fromEntries(
    Object.entries(comparisons).map(([id, comparison]) => [
      id,
      passBySurface(comparison),
    ])
  );
  const targetPasses = Object.fromEntries(
    targets.map((target) => [
      target.id,
      targetPass(captures[target.id], routes[target.id]),
    ])
  );
  const footerPass = {
    local: footerMatchesLive(captures.local, captures.live),
    ...(captures.deployed
      ? { deployed: footerMatchesLive(captures.deployed, captures.live) }
      : {}),
  };
  ...
  return {
    ...
    ok:
      Object.values(strictPass).every((bySurface) =>
        Object.values(bySurface).every(Boolean)
      ) &&
      Object.values(footerPass).every((bySurface) =>
        Object.values(bySurface).every(Boolean)
      ) &&
...
const run = async () => {
  ...
  const targets = [
    {
      baseUrl: process.env.LIVE_URL ?? "https://minshot.fehey.com",
      id: "live",
    },
    { baseUrl: process.env.LOCAL_URL ?? "http://127.0.0.1:4173", id: "local" },
    ...(process.env.DEPLOYED_URL
      ? [{ baseUrl: process.env.DEPLOYED_URL, id: "deployed" }]
      : []),
  ];
  ...
  try {
    ...
    const routes = Object.fromEntries(
      await Promise.all(
        targets.map(async (target) => [target.id, await checkRoutes(target)])
      )
    );
